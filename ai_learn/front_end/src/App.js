...
const App = () => {
    ...
    const handleWordSelect = async () => {
      ...
      if (selection && selection.toString().trim().length > 0) {
        ...
        if (words.length === 1) {
          ...
          const response = await axios.post('Backend-env.eba-gz7kcc7n.ap-southeast-1.elasticbeanstalk.com:80/translate-text', { text: selectedWord })
  ...
  const handleImageUpload = async () => {
    try {
      ...
      const response = await axios.post('Backend-env.eba-gz7kcc7n.ap-southeast-1.elasticbeanstalk.com:80/extract-text', formData);
  ...
  const handleTextToSpeech = async () => {
    try {
      const response = await axios.post('Backend-env.eba-gz7kcc7n.ap-southeast-1.elasticbeanstalk.com:80/synthesize-speech', { text: speechText, voiceId: 'Joanna' }, { responseType: 'arraybuffer' });
  ...
  const handleTextTranslate = async () => {
    try {
      const response = await axios.post('Backend-env.eba-gz7kcc7n.ap-southeast-1.elasticbeanstalk.com:80/translate-text', { text: sourceText });
